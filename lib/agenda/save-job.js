...
module.exports = async function (job) {
  try {
    ...
    if (id) {
      ...
      const result = await new Promise((resolve, error) => {
        this._collection.update(
          {_id: id}, update, {returnUpdatedDocs: true},
          (err, numAffected, affectedDocuments, upsert) => {
            err ? error(err) : resolve(affectedDocuments)
          }
        )
      });
    ...
    if (props.type === 'single') {
      ...
      const result = await new Promise((resolve, error) => {
        this._collection.update({name: props.name, type: 'single'}, update, {upsert: true, returnUpdatedDocs: true},
          (err, numAffected, affectedDocuments, upsert) => {
            if (err)
              error(err);
            else
    ...
    if (unique) {
      ...
      const exists = await new Promise((resolve, error) => {
        this._collection.find(query, (err, job) => {
          err ? error(err) : resolve(job[0])
        })
      });

      if (exists && uniqueOpts && uniqueOpts.insertOnly)
        return processDbResult(job, exists);

      const result = await new Promise((resolve, error) => {

        this._collection.update(query, update, {upsert: true, returnUpdatedDocs: true},
          (err, numAffected, affectedDocuments, upsert) => {
            if (err)
              error(err);
            else
    ...
    const result = await new Promise((resolve, error) => {
      this._collection.insert(props, (err, result) => {
        if (err)
          error(err);
        else
          resolve(result);
